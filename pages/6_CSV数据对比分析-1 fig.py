...
def plot_data(df1, df2, plot_type, x_column, y1_columns, y2_columns, show_markers):
    fig = go.Figure()
    ...
    for y1_column in y1_columns:
        if plot_type == 'scatter':
            fig.add_trace(go.Scatter(x=df1[x_column], y=df1[y1_column], mode='markers', name=y1_column, yaxis='y1'))
        elif plot_type == 'line':
            fig.add_trace(go.Scatter(x=df1[x_column], y=df1[y1_column], mode=mode, name=y1_column, yaxis='y1'))

    for y2_column in y2_columns:
        if plot_type == 'scatter':
            fig.add_trace(
                go.Scatter(x=df2[x_column], y=df2[y2_column], mode='markers', name=y2_column, xaxis='x2', yaxis='y2'))
        elif plot_type == 'line':
            fig.add_trace(
                go.Scatter(x=df2[x_column], y=df2[y2_column], mode=mode, name=y2_column, xaxis='x2', yaxis='y2'))

    # 计算 x1 和 x2 的最小值和最大值
    x_min = min(df1[x_column].min(), df2[x_column].min())
    x_max = max(df1[x_column].max(), df2[x_column].max())

    fig.update_layout(
        template='none',
        title=f'({get_ylabel(y1_columns)}-csv1 / {get_ylabel(y2_columns)}-csv2) ~ {x_column}',
        xaxis=dict(title=x_column, tickmode='auto', nticks=10, showgrid=True, showline=True, linewidth=1.5,
                   linecolor='black', range=[x_min, x_max]),
        xaxis2=dict(showticklabels=False, tickmode='auto', nticks=10, showgrid=True, showline=True, linewidth=1.5,
                    anchor='y2', overlaying='x', side='top', range=[x_min, x_max]),
        yaxis=dict(title=get_ylabel(y1_columns), showgrid=True, showline=True, linewidth=1.5, linecolor='black'),
        yaxis2=dict(title=get_ylabel(y2_columns), overlaying="y", side="right", showgrid=True, showline=True, linewidth=1.5, linecolor='black'),
...
# 示例用法
# df1 = pd.read_csv('path_to_csv1.csv')
# df2 = pd.read_csv('path_to_csv2.csv')
# fig = plot_data(df1, df2, 'line', 'x_column', ['y1_column1', 'y1_column2'], ['y2_column1', 'y2_column2'], show_markers=True)
# fig.show()
...
def data_plotly():
    ...
    if uploaded_file_1 is not None and uploaded_file_2 is not None:
        ...
        if y1_columns_1 or y2_columns_1 or y1_columns_2 or y2_columns_2:
            with col_33:
                show_markers = st.checkbox("显示数据点", value=False)
            y1_columns = y1_columns_1+y2_columns_1
            y2_columns = y1_columns_2+y2_columns_2
            fig1 = plot_data(df1,df2, plot_type, x_column_1, y1_columns, y2_columns, show_markers)
